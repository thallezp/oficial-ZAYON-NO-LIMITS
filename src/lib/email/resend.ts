...
export async function sendInviteEmail({
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #0a0d1a; color: #f3f4f6; border-radius: 12px; border: 1px solid #1f2937;">
      <h2 style="color: #3b82f6; border-bottom: 1px solid #1f2937; padding-bottom: 10px;">NEXUS Workspace OS</h2>
  ...
  if (!resend) {
    ...
    console.log("Assunto: Convite para NEXUS Workspace OS");
    console.log("Conteúdo:", html);
    return { success: true, mocked: true };
  }

  const { data, error } = await resend.emails.send({
    from: "NEXUS Workspace OS <onboarding@resend.dev>",
    to: email,
    subject: "Convite para se juntar à equipe no NEXUS HQ",
    html,
  });
...
export async function sendReminderEmail({
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #0a0d1a; color: #f3f4f6; border-radius: 12px; border: 1px solid #1f2937;">
      <h2 style="color: #ef4444; border-bottom: 1px solid #1f2937; padding-bottom: 10px;">Lembrete NEXUS</h2>
      ...
      <hr style="border: 0; border-top: 1px solid #1f2937; margin: 20px 0;" />
      <p style="font-size: 11px; color: #9ca3af; text-align: center;">NEXUS Workspace OS — Sistema Operacional Interno</p>
    </div>
  `;
  ...
  const { data, error } = await resend.emails.send({
    from: "NEXUS Lembrete <onboarding@resend.dev>",
    to: email,
    subject: title,
    html,
  });
